...
class User(object):
	...
	def Broke(self, s):
		sendMessage(s, self.getName() + " just went broke! D= you have lost your job and are now unemployed again")
	...
	def CDUpdate(self):
		for i in range(len(self.cooldown)):
			if self.cooldown[i][1] == 10:
				log(self.getName() + " cooldown 10 before")
			self.cooldown[i][1] -= 1
		removeE = []
		for CaCI in range(len(self.cooldown)):
			if self.cooldown[CaCI][1] == 0:
				removeE.append(self.cooldown[CaCI])
		try:
			for entry in removeE:
				self.cooldown.remove(entry)
		except:
			log("CD doesn't exsist?")
	...
	def update(self, s):
		...
		if self.downtime == 10:
			...
			log(self.getName() + ": CILM:" + str(len(self.CILM)) + "  EXP:+" + str(addEXP) + "  coins: " + str(self.getIncome()) + " - " + str(self.getExpenses()) + " = " + str(self.getIncome() - self.getExpenses()))

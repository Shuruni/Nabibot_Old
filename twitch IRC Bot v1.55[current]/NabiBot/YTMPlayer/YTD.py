...
npSongDir = "GUI/NabiPlayerNP.nabi"
mode = "default"
...
def progressBarUpdate():
	length = 47.5
	songLength = float(player.get_length())
	fails = 0
	while(songLength == 0):
		fails+=1
		songLength = float(player.get_length())
	if fails > 0:
		print("Progress Bar failed to read song length " + str(fails) + " times")
	timePerChar = float(songLength) / length

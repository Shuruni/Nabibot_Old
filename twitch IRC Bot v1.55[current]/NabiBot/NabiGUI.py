...
topImg = ImageTk.PhotoImage(file="GUI/GUIPreview.png")
...
class ExpressionsMenu(object):
	def __init__(self):
		...
		for i in self.available:
			tRaw = PIL.Image.open("Expressions/NabiLaunchboard128x128/resized/" + i.strip() + ".png")
...
def playPause(arg):
	if player.is_playing():
		player.pause()
		with open("YTMPlayer/CurSong.txt", "w") as f:
			f.write(u"")
			f.truncate()
	else:
		player.play()
...
#images
MainBGRAW = PIL.Image.open("GUI/starry.png")
MainBGRAW = MainBGRAW.resize((1080,753), PIL.Image.ANTIALIAS)
MainBGIMG = ImageTk.PhotoImage(MainBGRAW)

MainNabiRAW = PIL.Image.open("Expressions/NabiV2/12.png")
MainNabiRAW = MainNabiRAW.resize((190,300), PIL.Image.ANTIALIAS)
MainNabiIMG = ImageTk.PhotoImage(MainNabiRAW)

MainNabi2RAW = PIL.Image.open("Expressions/NabiV2/00.png")
MainNabi2RAW = MainNabi2RAW.resize((190,300), PIL.Image.ANTIALIAS)
MainNabi2IMG = ImageTk.PhotoImage(MainNabi2RAW)

MainNabi3RAW = PIL.Image.open("Expressions/NabiV2/eh7.png")
MainNabi3RAW = MainNabi3RAW.resize((190,300), PIL.Image.ANTIALIAS)

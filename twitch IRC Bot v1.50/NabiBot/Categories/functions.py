...
class debug(object):
	def exit(self, s, options, user, ul, comList):
		...
		sys.exit()
	def crash(self, s, options, user, ul, comList):
		raise ForceCrash("Don't worry, Nabi was prepared! ^w^")
...
class dummy(object):
	...
	def addOption(self, s, options, user, ul, comList):
		result = comList.isDummy(options[2])
		if result != False:
			result.addOption(options[1])
			sendMessage(s, "Option successfully added to " + result.getName() + " (^o^)/")
		else:
			sendMessage(s, "/w " + user.getName() + " " + options[2] + " isn't a Dummy Command (>w<)/")
	def removeOption(self, s, options, user, ul, comList):
		result = comList.isDummy(options[2])
		if result != False:
			removed = result.remOption(options[1])
			if removed:
				sendMessage(s, "Option successfully removed from " + result.getName() + " (-o-)/")
			else:
				sendMessage(s, "/w " + user.getName() + " " + options[1] + " isn't an option in " + result.getName() + " (>w<)/")
		else:
			sendMessage(s, "/w " + user.getName() + " " + options[2] + " isn't a Dummy Command (>w<)/")
...
class Nabi(object):
	...
	def NPChange(self, s, options, user, ul, comList):
		with open("../../Plugins/SC/Files/np_listening.txt", "w") as f:
			if options[len(options)-1].lower().strip() == "none":
				f.write("")
			else:
...
class Osu(object):
	def NP(self, s, options, user, ul, comList):
		with open('../../Plugins/FoobarNPSave/FoobarNP.txt', 'r') as f:
			foobar = f.read()
		if foobar == u"":
			with open('../../Plugins/SC/Files/np_playing.txt', 'rb') as p:
				playing = p.readline().strip()
				if playing == "":
					with open('../../Plugins/SC/Files/np_listening.txt', "rb") as l:
					...
					with open('../../Plugins/SC/Files/np_playing_DL.txt', 'rb') as p:
						link = p.readline().strip()
						sendMessage(s, "Shuruni-Oniichan is playing: " + playing + " " + link + " (^o^)/")
		else:
			sendMessage(s, "Shuruni-Oniichan is listening to: " + foobar + " ^w^")
		return True

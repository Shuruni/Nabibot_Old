...
class User(object):
	...
	def newComment(self):
		...
		print(self.getName() + "'s comments: " + str(len(self.CILM)))
	...
	def canUseCommand(self, command):
		...
		for i in self.cooldown:
			if i[0] == command:
				return False
		print command.getFunction()
		# print self.getStanding()
		print "restriction present: " + str(self.getStanding().hasRestrictions(command.getFunction() + ")"))

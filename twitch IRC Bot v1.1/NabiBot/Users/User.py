...
class User(object):
	...
	def update(self, s):
		...
		if self.downtime == 10:
			...
			pass
		elif len(self.CILM) >= 1:
			addEXP = len(self.CILM)*self.ReturnsMult
			self.exp += int(addEXP)
			...
			self.exp += 1
			self.wallet.updateWallet()
			print(self.getName() + " gained 1 exp, was paid " + str(self.getIncome()) + "coins and spent " + str(self.getExpenses()) + "coins on expenses")
...
def blockedUsers(file):
	...
	with open(file, 'rb') as csvfile:
		...
		for row in reader:
			...
			usersLoaded += 1
			currentRow += 1
	print(str(usersLoaded) + " blocked users loaded")
	return users[:]

...
class dummy(object):
	...
	def add(self, s, options, user, ul, comList):
		if user.getStanding().currentStanding() == "Veteran":
			sendMessage(s, "Nabi is tired, this will be implemented later (-o-)zzZ")
...
class level(object):
	def level(self, s, options, user, ul, comList):
		if options == []:
			...
			self.level(s, [], ul.isInList(options[0]), ul, comList)
class Nabi():
	def __init__(self):
		self.messages = {
	...
	def Expression(self, s, options, user, ul, comList):
		if options == []:
			sendMessage(s, "Nabi can change her expression to any of these: " + str(self.AE))
		elif options[0].lower() in self.AE:
